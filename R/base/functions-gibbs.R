...
samp.move=function(paramz){
  ...
  if (p == 1) {
    ...
    # death prob 2 -> 1 is (1/3) and birth prob 1 -> 2 is 1. 
    p0=1/3 
  }
  if (p == maxp) {
    if (z < 1/2) {
      indin.new=death(paramz$indin)
      # birth prob T-1 -> T is (1/3) and death prob T -> T-1 is 1/2
      p0=2/3 
    }
  ...
  if (1 < p & p < maxp) {
    if (z < 1/3) {
      indin.new=birth(paramz$indin,paramz$indout)
      # death prob from T -> T-1 is (1/2) and birth prob from T-1 -> T is (1/3)
      if (p==maxp-1) p0=3/2 
    }
    if (1/3 < z & z < 2/3) {
      indin.new=death(paramz$indin)
      # birth prob from 1 -> 2 is 1 and death prob from 2 -> 1 is 1/3
      if (p==2) p0=3 
    }
  ...
  indout.new=seq1[k]
  if (z<prob) return(list(xmat=xmat.orig[,indin.new],indin=indin.new,
                          indout=indout.new))
  return(list(xmat=xmat.orig[,indin.old],indin=indin.old,indout=paramz$indout))
}
#------------------------------
death=function(indinz){
  if (length(indinz)==2) return(indinz[2]) # cannot delete intercept
